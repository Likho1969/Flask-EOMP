...
def init_post_table():
    with sqlite3.connect('my_database.db') as conn:
        conn.execute("CREATE TABLE IF NOT EXISTS post (product_id INTEGER PRIMARY KEY AUTOINCREMENT,"
...
def create_blog():
    ...
    if request.method == "POST":
        ...
        product_price = request.form['content']
        product_type = request.form['content']
        ...
        with sqlite3.connect('my_database.db') as conn:
            ...
            cursor.execute("INSERT INTO post("
                           ...
                           "product_price,"
                           "product_type,"
                           "date_created) VALUES(?, ?, ?, ?)", (product_name, product_price, product_type, date_created))
            conn.commit()
            response["status_code"] = 201
